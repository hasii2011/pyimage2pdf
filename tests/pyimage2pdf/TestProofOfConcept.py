...
class TestProofOfConcept(UnitTestBase):
    ...
    def _scalePages(self, filePath: Path, factor: float = 0.1):
        ...
        rect.left   = FloatObject(20.0)
        rect.right  = FloatObject(200.0)
        rect.top    = FloatObject(newHeight - 2.0)
        rect.bottom = FloatObject(newHeight - 50.0)

        creationDate: datetime = cast(DocumentInformation, pdfReader.metadata).creation_date    # type: ignore
        annotation:   FreeText = FreeText(
            text=f'Created by Pyut 9.6.0 - {creationDate}',
            rect=rect,
            font="PT Mono",
            bold=True,
            italic=False,
        ...
        pdfWriter.metadata = pdfReader.metadata
